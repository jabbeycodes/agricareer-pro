...
import { getResumeText, PROFILE } from './profile'

const ANTHROPIC_URL = 'https://api.anthropic.com/v1/messages'

export async function tailorResume(job: Job): Promise<string> {
...
Return ONLY the tailored resume text.`
  ...
  try {
    ...
    return data?.content?.[0]?.text || fallbackResume(job)
  } catch {
    return fallbackResume(job)
  }
}

function fallbackResume(job: Job): string {
